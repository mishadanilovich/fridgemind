...
// Realtime-синхронизация с двух аккаунтов одного household (CLAUDE.md §6 и §8, сценарий
// "один дома, другой в магазине"): второй участник присоединяется по реальной
// ссылке-приглашению, добавляет ручную позицию — первый видит её без перезагрузки
// (Supabase Realtime → router.refresh(); RLS проверяется на JWT каждого из двух аккаунтов).
test("shopping realtime: два аккаунта household видят изменения друг друга без перезагрузки", async ({
  browser,
}) => {
  // Помимо двух логинов сценарий создаёт второго пользователя через admin API и прогоняет
  // его по реальному инвайт-потоку (/invite/{code} → редирект на «Сегодня»). На холодном
  // .next (маршруты компилируются по пути) 120с впритык — держим тот же запас, что и в stage-8.
  test.setTimeout(180_000);
  const userA = readTestUser();
  const manualName = `E2E реалтайм ${Date.now()}`;

  async function login(ctx: Awaited<ReturnType<typeof browser.newContext>>, user: TestUser) {
  ...
  const userB = await createSecondaryUser();
  const ctxA = await browser.newContext();
  const ctxB = await browser.newContext();
  try {
    const pageA = await login(ctxA, userA);
    const pageB = await login(ctxB, userB);

    // Пользователь B входит в семью A по приглашению — реальный поток из раздела 6 CLAUDE.md.
    const inviteCode = await getInviteCodeFor(userA.id);
    await pageB.goto(`/invite/${inviteCode}`);
    await pageB.getByRole("button", { name: "Присоединиться" }).click();
    await expect(pageB.getByRole("heading", { name: "Сегодня", level: 1 })).toBeVisible({
      timeout: 30_000,
    });
    ...
    await ctxA.close();
    await ctxB.close();
    await deleteSecondaryUser(userB);
  }
});
