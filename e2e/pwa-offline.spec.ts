...
test("экраны кэшируются в Dexie и читаются офлайн через /~offline", async ({ page, context }) => {
  ...
  await test.step("добавление в запасы пишет снапшот инвентаря", async () => {
    ...
    await waitForSnapshot(page, "pantry", "all");
  });
  ...
  await test.step("/~offline рендерит сохранённые запасы", async () => {
    await page.goto("/~offline?from=/inventory");
    await expect(page.getByRole("heading", { name: "Запасы", level: 1 })).toBeVisible();
    await expect(page.getByText(INGREDIENT_NAME)).toBeVisible();
    await expect(page.getByText("500 г")).toBeVisible();
  });
  ...
  await test.step("офлайн-мутация показывает ошибку сети, а не роняет экран", async () => {
    await page.goto("/menu");
    await expect(page.getByRole("heading", { name: "Меню на неделю", level: 1 })).toBeVisible();

    await context.setOffline(true);
    await page.getByRole("button", { name: "Завтрак" }).first().click();
    await page.getByText(RECIPE_TITLE, { exact: true }).click();
    await page.getByRole("button", { name: "Добавить в день" }).click();
    await expect(page.getByText(/Нет соединения с сервером/)).toBeVisible();
    await context.setOffline(false);
  });

  // Прогон делит один household со всеми спеками (общий globalSetup/teardown), а
  // upsertPantryItemQuantity суммирует повторные добавления. Без очистки эти +500 г остаются
  // в pantry_items и ломают абсолютные проверки shopping.spec.ts ("200 г"/"300 г"), который
  // идёт после этого файла по алфавиту. Удаляем строку целиком — как в inventory.spec.ts.
  // Важно: до сброса cookie ниже — удаление требует активной сессии.
  await test.step("очистка: удалить добавленную позицию запасов", async () => {
    await page.goto("/inventory");
    const listRow = page
      .locator("section")
    ...
    // Подтверждение: шит переключается на "Удалить из запасов?" с повторной кнопкой.
  ...
  // Снапшоты остаются в Dexie после ухода cookie (кэш чистится только явным выходом),
  // но без auth-cookie /~offline их не показывает — общий телефон, человек без входа.
  await test.step("/~offline без auth-cookie не показывает снапшоты", async () => {
    await context.clearCookies();
    await page.goto("/~offline?from=/recipes");
    await expect(page.getByText("Войдите в аккаунт")).toBeVisible();
    await expect(page.getByText(RECIPE_TITLE, { exact: true })).toBeHidden();
  });
});
